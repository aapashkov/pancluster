...
if ! command -v trim_galore > /dev/null; then
  printf '%s\n' "error: trim_galore: command not found" >&2
  exit 1
fi
...
if [[ -z "${3+x}" ]]; then
  trim_galore --gzip \
    ...
    --no_report_file \
    "$file" >&2
  chmod -v 666 "${tmp}/${base}_trimmed.fq.gz" >&2
  mv -v "${tmp}/${base}_trimmed.fq.gz" "${outprefix}.fastq.gz" >&2
...
# If two READs were passed, run paired-end trimming
  ...
  trim_galore --gzip \
    ...
    --paired \
    "$file" "$file2" >&2
  chmod -v 666 "${tmp}/${base}_val_1.fq.gz" "${tmp}/${base}_val_2.fq.gz" >&2
  mv -v "${tmp}/${base}_val_1.fq.gz" "${outprefix}_1.fastq.gz" >&2
  mv -v "${tmp}/${base}_val_2.fq.gz" "${outprefix}_2.fastq.gz" >&2
fi
